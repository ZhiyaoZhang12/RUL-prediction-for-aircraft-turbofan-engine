...
class RNNModel(nn.Module):
    def __init__(self,
        ...
        if self.cell_type == 'LSTM':
            self.encoder_cell = nn.LSTM(self.input_size, self.hidden_size, self.num_layers, batch_first=True, dropout = self.dropout_train)
        ...
        if self.cell_type == 'RNN':
            self.encoder_cell = nn.RNN(self.input_size, self.hidden_size, self.num_layers, batch_first=True, dropout = self.dropout_train)
        # if self.cell_type == 'CNN':
        #     self.encoder_cell = nn.CNN(self.input_size, self.hidden_size, self.num_layers, batch_first=True)
...
class RNNTrainer(Trainer):
    def __init__(self,
        ...
        if loss_function == 'MSE':
            self.criterion = nn.MSELoss()    #x**2 - y**2
